...
function render (data, options) {
  ...
  const image = options && options.image || false
  const format = options && options.format || 'svg'

  const renderedData = renderGraph(data, { image: image })
  const dot = renderedData.dot
  const imageData = renderedData.imageData
  
  if (format === 'dot') return dot

  const svg = require('viz.js')(dot, { format: 'svg', engine: 'dot', images: imageData })

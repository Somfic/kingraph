...
function render (data, options) {
  ...
  return { dot: dot, imageData: options && options.image ? renderImages(data) : [] }
...
/*
 * Generate Image list data for viz.js
 */
 function renderImages(data) {
   const people = data.people || {}
   return values(map(people, (p, id) => setImageData(p))).filter(hasImage)
 }
 ...
 function setImageData(data) {
   return { path: data.image, height: "50px", width: "50px" }
 }
